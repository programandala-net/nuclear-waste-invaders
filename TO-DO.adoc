...
- Erode the wall in `?<break-wall` and `?break-wall>`.  Reuse
  `break-bricks>` and `<break-bricks` to print the three eroded bricks
  of the breach.
...
- Fix: the ticks clock is reset in `prepare-attack`, but
  `mothership-time` is reset a bit later in `attack-wave`. That seems
  to be the reason sometimes the mothership does not appear at the
  start of a second attack. Solution: Move `reset-dticks` to the end
  of `attack-wave`.
