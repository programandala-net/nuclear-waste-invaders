...
Fix: wall detection fails after any invader has been wounded.  The bug
seems to be caused by `turn-back`.

The following words have been tested and discarded as causes of the
bug: `explode`, `wounded`, `set-flying-sprite`. I.e. the bug remains
when those words are deactivated.
...
- Fix: `flying-to-the-left?` is not updated in `set-invader`.
- Use `3*`.
