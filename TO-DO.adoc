...
- Recharge only one projectile.
- Fix: updating the ammo while recharging bullets is too much work, it
  halts the rest of game. The recharging could be an independent
  action.
- Highlight also the ammo of the current gun, not only the icon.
- Remove `set-exploding-projectile-sprite`, which is used only once.
- Remove `set-beaming-mothership-sprite`, which is used only once.
- Remove `set-exploding-mothership-sprite`, which is used only once.
- Use a different color to highlight the current gun, or inverse
  video, not just bright.
- Fix: timings.
- Fix: eventual crash in the second level.
