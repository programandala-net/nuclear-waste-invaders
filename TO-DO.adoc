...
- Animate the invader explosions with several frames.
...
- Improve mothership explosion: Change the color for every frame.
- Improve mothership explosion: Make frames a logical sequence.
- Fix: the mothership explosion can be impacted.
...
- Replace the run-time calculation done in `hit-wall?` and
  `prepare-wall` with a table of constants constants calculated at
  compile-time.
