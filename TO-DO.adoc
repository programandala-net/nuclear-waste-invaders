...
- Design sprites for the balled invaders.

== 2018-02-19

- Replace `<wall-ball-frames` and `wall>-ball-frames` with
  `wall-ball-frames`, because they are identical; add a compile-time
  check.
- Rename `udg/tank` `tank-width`, and so all similar cases.
- Fix: When the ammo of the current gun is initiated, the updating is
  displayed on the status bar, before the status bar is displayed.
