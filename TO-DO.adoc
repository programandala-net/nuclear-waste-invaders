...
- Improve compilation of `controls` with a word that replaces all
  `c,`.
- Improve effect of missiles.
- Make the new direction of the mothership depend also on the number
  of missiles left: when there's no missile left, the 
  position of the tank is removed from the calculation.
