...
Suddenly `localized-string` fails with error `#-4` (empty stack)
because strings containing Esperanto or Spanish characters (any
character above 127) are not compiled by `,"`. This didn't happen
before.

At first it seems the problem should be in Solo Forth's `scan` or
`parse`, but these are OK.

Then `localized-string` and all related words have been moved to the
library of Solo Forth, and the very same code works find there.
